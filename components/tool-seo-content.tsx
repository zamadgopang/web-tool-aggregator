...
export function ToolSeoContent({ tool }: ToolSeoContentProps) {
    ...
    return (
        <section className="mt-8 sm:mt-12 mb-12 sm:mb-16 max-w-4xl mx-auto px-3 sm:px-4 space-y-8 sm:space-y-12" aria-label={`About ${tool.title}`}>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6 p-4 sm:p-6 rounded-2xl bg-white/40 dark:bg-black/40 backdrop-blur-2xl dark:backdrop-blur-2xl backdrop-saturate-150 dark:backdrop-saturate-150 border border-white/60 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)]">
                ...
                <div>
                    ...
                    <ol className="space-y-2 sm:space-y-3">
                        {seoContent.howItWorks.map((step, index) => (
                            <li key={index} className="flex items-start gap-2 sm:gap-3 p-2.5 sm:p-3 rounded-xl bg-white/60 dark:bg-white/10 backdrop-blur-md dark:backdrop-blur-md border border-white/80 dark:border-white/20 shadow-sm dark:shadow-sm">
            ...
            <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                    <div className="flex items-start gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl bg-white/60 dark:bg-white/10 backdrop-blur-md dark:backdrop-blur-md border border-white/80 dark:border-white/20 shadow-sm dark:shadow-sm">
            ...
            {keywords.length > 0 && (
                <div className="flex flex-wrap gap-2">
                    {keywords.map((keyword, index) => (
                        <span
                            key={index}
                            className="inline-flex items-center rounded-full bg-white/60 dark:bg-white/10 backdrop-blur-md dark:backdrop-blur-md border border-white/80 dark:border-white/20 shadow-sm dark:shadow-sm px-3 py-1 text-xs text-muted-foreground"
                        >
                            {keyword}
                        </span>
                    ))}
